...
RepTimer reconnaît deux formats dont les effets sont volontairement différents.
...
| Sauvegarde v2 | Restauration complète après confirmation | Remplacées par celles de la sauvegarde | Historique et trois préférences exportables remplacés |

## Exporter une sauvegarde v2

Choisir **Exporter** construit puis partage un fichier JSON v2 contenant :

- toutes les séances, y compris les groupes à répétitions variables et leurs
  valeurs dormantes ;
- tout l'historique ;
- le thème, le préremplissage du nom des exercices et le mode de notification ;
...
Le schéma détaillé est décrit dans le [contrat de sauvegarde v2](backup-v2.md).
...
## Restaurer une sauvegarde v2

Après la sélection d'un fichier v2 valide, RepTimer affiche un résumé avec sa
...
préremplissage du nom et le mode de notification. Le thème est appliqué
...
- vérifier le résumé avant de confirmer une restauration v2 ;
